capture program drop csdid_estadd

program define csdid_estadd
syntax anything, statname(name) [format(string) omitstars store store_filename(string) store_options(string) store_group(string)]
	...
	* Retrieve stored estimates from r(table)
	...
	if "`e(cmd)'" == "eventstudyinteract" {
		...
		local t = r_table[rownumb(r_table,"z"),colnumb(r_table,"`anything'")]
		local pvalue = r_table[rownumb(r_table,"pvalue"),colnumb(r_table,"`anything'")]
		assert `pvalue' ~= .
